...
try:
    html = urllib.request.urlopen(url, context=ctx).read() # Open and reading the url
    soup = BeautifulSoup(html, 'html.parser') # Passes the data to the BeautifulSoup parser
    tags = soup('p')
    for tag in tags: # Retrieve all of the tags
        count += 1
except:
